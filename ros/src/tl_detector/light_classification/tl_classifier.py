...
IMAGE_PATH_SIM = os.path.dirname(os.path.realpath(__file__)) + '/../../../../saved_images/simulator/'
IMAGE_PATH_SITE = os.path.dirname(os.path.realpath(__file__)) + '/../../../../saved_images/site/'
SAVE_IMAGES = False
MAX_IMAGE_WIDTH = 300
...
class TLClassifier(object):
    def __init__(self):
        ...
        model_path = os.path.dirname(os.path.realpath(__file__)) + self.config['model_path']
    ...
    # only for debug or re-train model
    def save_image(self, image, light_class):
        bgr_image = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)
        if self.config['is_site']:
            cv2.imwrite(os.path.join(IMAGE_PATH_SITE, "image_%04i_%d.jpg" % (self.image_counter, light_class)), bgr_image)
        else:
            cv2.imwrite(os.path.join(IMAGE_PATH_SIM, "image_%04i_%d.jpg" % (self.image_counter, light_class)), bgr_image)
